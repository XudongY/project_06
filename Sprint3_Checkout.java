...
public class Sprint3_Checkout {
    ...
    public static HashMap<String, List<String>> check_List(List<Indivdual> indivdualList, List<Family> familyList) {
        ...
        for (Family fam : familyList) {
            ...
            for (int i = 0; i < indivdualList.size(); ++i) {
                String ID=indivdualList.get(i).getID();
                Date Deathday=indivdualList.get(i).getDeath();
                Date Birthday=indivdualList.get(i).getBirthday();
                if(Deathday!=null)
                {
                   check_for_upcoming_birthdays(ID,Birthday);
                }
    ...
    //us38 and us39 are  done  by DISHA
    public static boolean check_for_upcoming_birthdays(String ID, Date Birthday)
    {
        try{

            SimpleDateFormat format=new SimpleDateFormat("dd MM yyyy");
            Calendar cal=Calendar.getInstance();
            String current_date_String_Format=format.format(cal.getTime());
            Date current_date_Date_Format=format.parse(current_date_String_Format);
            cal.add(Calendar.DATE, +30);
            String forward_date_String_Format=format.format(cal.getTime());
            Date forward_date_Date_Format=format.parse(forward_date_String_Format);

            if((Birthday.before(forward_date_Date_Format))&&(Birthday.after(current_date_Date_Format))){
    ...
    public static void us38_list_upcoming_birthdas(HashMap map) {
        ...
        while (mapIterator.hasNext()) {
            ...
            String d = new SimpleDateFormat("dd MMM yyyy").format(value);
            String error = "PRINT: INDIVIDUAL: US38: " + keyValue
                    + ": Birthday s upcoming " + d + " in recent 30 days.";
