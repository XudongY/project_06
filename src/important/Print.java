			...
			if (obj.getDivorced() != null) {
				System.out
						.format("%5s %1s%1s %12s%2s %12s%2s %1s%1s %16s%4s %1s%1s %16s%4s %72s%1s",
								"|", obj.getID(), "|", sdf.format(marriage),
								"|", sdf.format(divorce), "|",
								obj.getHusbandID(), "|", obj.getHusbandName(),
				...
				System.out
						.format("%5s %1s%1s %12s%2s %12s%2s %1s%1s %16s%4s %1s%1s %16s%4s %72s%1s",
								"|", obj.getID(), "|", sdf.format(marriage),
								"|", "NA", "|", obj.getHusbandID(), "|",
								obj.getHusbandName(), "|", obj.getWifeID(),
								"|", obj.getWifeName(), "|",
								String.valueOf(obj.getChildren()), "|");
			}
			System.out.println();
	...
	public static void print_arraylist_indivdual(
		...
		for (int i = 0; i < tobedisplayed.size(); i++) {
			...
			System.out.format(
					...
					"|",
					obj.getChild().size() == 0 ? "NA" : String.valueOf(obj
							.getChild()),
					"|",
					obj.getSpouse().size() == 0 ? "NA" : String.valueOf(obj
							.getSpouse()), "|");
			System.out.println();
		}
